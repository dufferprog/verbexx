...
int verb_left_assign(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    ...
    auto unshare_rc = get_right_keyword(vexpr, L"unshare");                                                             // r/c = -1 if not present   
                                                                                                                    
    auto lsize = vexpr.lparms.values.size();                                                                            // number of variables to set
    auto rsize = vexpr.rparms.values.size();                                                                            // number of available values
    ...
    {
        results = to_results(vexpr.rparms.values.at(0));                                                               // output value = single value assigned to single variable
    }
...
int verb_right_assign(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    ...
    auto lsize = vexpr.lparms.values.size();                                                                            // number of available values
    auto rsize = vexpr.rparms.values.size();                                                                            // number of variables to set
    ...
    {
        results = to_results(vexpr.lparms.values.at(0));                                                               // output value = single value assigned to single variable
    }
    else                                                                                                               // 0 or more than 1 value -- need to pass back multiple results
    {
        vlist_S vlist { };                                                                                             // clean results vlist -- no unshare: keyword, etc. 
        vlist.values = vexpr.lparms.values;                                                                            // copy over just the left-side positional parms = all values to be assigned   
        results = to_results(vlist_val(vlist));                                                                        // convert vlist to results
        results.multiple_results = true;                                                                               // pass back vlist as multiple results
    }
...
static int incr_decr(frame_S& frame, const e_vexpr_S& vexpr, results_S& results, bool do_incr, const std::wstring& opws) try
{
    value_S value { };
    ...
    if (vexpr.lparms.value_ct > 0)      // parms on left (postfix version)?
    {                                   
...
int verb_increment(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    ...
    return incr_decr(frame, vexpr, results, true, L"increment"); 
}
M_endf
...
int verb_decrement(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    // already known that there is one left-side or one right-side pre-defined non-constant identifier/lvalue, and no other parms

    M__(M_out(L"verb_decrement() called");)

    return incr_decr(frame, vexpr, results, false, L"decrement");
}
M_endf
...
static int get_eq_values(frame_S &frame, const e_vexpr_S& vexpr, value_S& lvalue, value_S& value1, value_S& value2, results_S& results, bool must_match = true) try
{
    value_S value0 {vexpr.lparms.values.at(0)};  // get unsubstituted value of left-side parm (save for error messages0)     
    lvalue = value0;                            // pass back variable name/lvalue

    value2 = vexpr.rparms.values.at(0);          // get right-side value -- should be a string, int64 or float64 value
    ...
    if (must_match)
    {
        ...
        M_out_emsg1(L"get_eq_values() -- verb=%s, left-side parm type (%S) does not match type of right-side parm (%S) -- cannot execute verb") % verb_name(vexpr) % type_str(value1.ty) % type_str(value2.ty);
    ...
    else                                                // exact match not required -- make sure substituted variable is in same class as right-side parm 
    {
        ...
        M_out_emsg1(L"get_eq_values() -- verb=%s, type left-side parm (%S) is not in same class as type of right-side parm (%S) -- cannot execute verb") % verb_name(vexpr) % type_str(value1.ty) % type_str(value2.ty);
...
int verb_add_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    ...
    auto grc = get_eq_values(frame, vexpr, lvalue, value1, value2, results);
...
int verb_subtract_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
...
int verb_multiply_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    // already known that there is one left-side defined variable positional parm, and one right-side arithmetic positional parm
...
int verb_divide_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    // already known that there is one left-side defined variable positional parm, and one right-side arithmetic positional parm
...
int verb_power_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    // already known that there is one left-side defined variable positional parm, and one right-side arithmetic positional parm
...
int verb_remainder_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
...
int verb_concatenate_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
...
int verb_bitand_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    // already known that there is one left-side defined variable positional parm, and one right-side integer positional parm
...
int verb_bitor_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    // already known that there is one left-side defined variable positional parm, and one right-side integer positional parm
...
int verb_bitxor_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    // already known that there is one left-side defined variable positional parm, and one right-side integer positional parm
...
int verb_shift_left_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    // already known that there is one left-side defined variable positional parm, and one right-side integer positional parm
    ...
    auto grc = get_eq_values(frame, vexpr, lvalue, value1, value2, results, false);
    if (grc != 0) return grc;                                                             // get_eq_values() should have already set appropriate results, if there was an error
...
int verb_shift_right_arithmetic_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    // already known that there is one left-side defined variable positional parm, and one right-side integer positional parm
...
int verb_shift_right_logical_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    // already known that there is one left-side defined variable positional parm, and one right-side integer positional parm
    ...
    if (is_value_signed(vexpr.lparms.values.at(0)))     
...
static int verb_compare_check(const e_vexpr_S& vexpr, results_S& results)
{
     if (is_same_class(vexpr.lparms.values.at(0), vexpr.rparms.values.at(0)))
         return 0; 

     // error -- the two values cannot be compared, since they are not the same class

      
     M_out_emsg1(L"verb__compare_check() -- verb=%s, left-side and right-side values cannot ne compared because they are not the same class") % verb_name(vexpr);
...
int verb_eq(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    ...
    M_verb_compare(value, verb__eq, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0), 1) 
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_ne(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    ...
    M_verb_compare(value, verb__ne, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0), 0)
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_lt(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    ...
    M_verb_compare(value, verb__lt, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0), 0) 
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_gt(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    ...
    M_verb_compare(value, verb__gt, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0), 0)
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_le(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    ...
    M_verb_compare(value, verb__le, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0), 1)
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_ge(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    ...
    M_verb_compare(value, verb__ge, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0), 1)
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_and(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    bool    tf    { };   

    // already known that there is one left int64 positional parm, and one right int64, vexpr, or identifier parm (that might need evaluating)
...
int verb_or(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    bool    tf    { };  
...
int verb_xor(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    M_verb_binary_fval_integer_integer(value, verb__xor, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0)) 
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_bitand(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    M_verb_binary_fval_integer(value, verb__bitand, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0)) 
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_bitor(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    M_verb_binary_fval_integer(value, verb__bitor, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0)) 
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_bitxor(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    M_verb_binary_fval_integer(value, verb__bitxor, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0)) 
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_bitnot(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    M_verb_unary_fval_integer(value, verb__bitnot, vexpr.rparms.values.at(0)) 
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_shift_left(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    M_verb_binary_fval_integer_integer(value, verb__shift_left, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0)) 
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_shift_right_arithmetic(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    M_verb_binary_fval_integer_integer(value, verb__shift_right_arithmetic, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0)) 
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_shift_right_logical(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    if (is_value_signed(vexpr.lparms.values.at(0)))     
        M_verb_binary_fval_signed_integer(  value, verb__shift_right_logical_1, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0)) 
    else
        M_verb_binary_fval_unsigned_integer(value, verb__shift_right_logical_2, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0)) 
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_add(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    M_verb_binary_fval_arith( value, verb__add, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0) ) 
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_minus(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    if (vexpr.lparms.value_ct == 0)   // unary minus? 
         M_verb_unary_fval_arith( value, verb__negate, vexpr.rparms.values.at(0) )
    else
         M_verb_binary_fval_arith( value, verb__subtract, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0) ) 
...
int verb_multiply(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    M_verb_binary_fval_arith(value, verb__multiply, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0)) 
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_divide(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    M_verb_binary_fval_arith(value, verb__divide, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0))    
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_power(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    M_verb_binary_fval_arith(value, verb__power, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0)) 
    results = to_results(value);     
    return 0;
}
M_endf
...
int verb_remainder(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    value_S value { };
    ...
    M_verb_binary_fval_integer(value, verb__remainder, vexpr.lparms.values.at(0), vexpr.rparms.values.at(0)) 
    results = to_results(value);         
    return 0;
}
M_endf
...
int verb_concatenate(frame_S& frame, const e_vexpr_S& vexpr, const verbdef_S& verbdef, results_S& results) try
{
    // already known that there is one left and one right string positional parm

    M__(M_out(L"verb_concatenate() called");)

    results = to_results(string_val(vexpr.lparms.values.at(0).string + vexpr.rparms.values.at(0).string)); 
    return 0; 
}
M_endf
